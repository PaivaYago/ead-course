...
public interface CourseRepository extends JpaRepository<CourseModel, UUID>, JpaSpecificationExecutor<CourseModel> {
    ...
    @Query(value="select case WHEN count(tcu) > 0 THEN true ELSE false " +
            "END FROM tb_courses_users tcu WHERE tcu.course_id= :courseId AND tcu.user_id= :userId;", nativeQuery = true)
